...
parser = argparse.ArgumentParser(
        description='''run anvi'o pangenomics pipeline on *.fna genome files''')
...
for file in os.listdir(input_path):
        if file.endswith(".fna"):
            ...
            anvio_reformat_call = "anvi-script-reformat-fasta "+input_path+file+" -l 1000 --simplify-names --prefix "+in_prefix+" -o "+output_path+"anvio_reformatted_fasta_files/"+in_prefix+"_clean.fna"
            subprocess.call(anvio_reformat_call, shell=True)
            # run anvi-gen-contigs-database to generate database from fasta files
            anvio_gen_contigs_call = "anvi-gen-contigs-database -f "+output_path+"anvio_reformatted_fasta_files/"+in_prefix+"_clean.fna"+" -o "+external_genome_dict[in_prefix]+" -n "+project_name+"_"+in_prefix+" -T "+w
...
# run anvi'o
run_anvio_call = "anvi-pan-genome -g "+output_path+project_name+"-GENOMES.db"+" -n "+project_name+" --output-dir "+output_path+"anvio_pangenome/"+" --num-threads "+w+" --minbit 0.5 --mcl-inflation 10 --use-ncbi-blast"
subprocess.call(run_anvio_call, shell=True)

# add ANI to the anvi'object
ani_call = "anvi-compute-genome-similarity --external-genomes "+output_path+"external_genomes"+ "--program fastANI --output-dir "+output_path+"ANI/ --num-threads "+w+" --pan-db "+output_path+"anvio_pangenome/"+project_name+"-PAN.db"
subprocess.call(ani_call, shell=True)